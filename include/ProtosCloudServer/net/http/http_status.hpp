...
template <>
struct fmt::formatter<ProtosCloudServer::http::HttpStatus> {
    static auto parse(format_parse_context& ctx){
        return ctx.begin();
    }

    template <typename FormatContext>
    auto format(ProtosCloudServer::http::HttpStatus status, FormatContext& ctx) const {
        return fmt::format_to(ctx.out(), "{} {}", static_cast<int>(status),
                                ProtosCloudServer::http::HttpStatusString(status));
    }
};